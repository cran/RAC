...
ClamF_pop_RKsolver <- function(Param, times, IC, Tint, Chlint){
  ...
  Tapp=Tint[t]
  Chlapp=Chlint[t]
  output<-ClamF_pop_equations(Param, Tapp, Chlapp, Ww[t])
  dw=unlist(output[1])
  k1=timestep*dw
  ...
  output<-ClamF_pop_equations(Param, Tapp$y, Chlapp$y, Ww[t]+k1/2)
  dw=unlist(output[1])
  k2=timestep*dw
  ...
  Tapp=approx(seq(from=1,to=tf,by=timestep),Tint,xout=(t+timestep/2))
  Chlapp=approx(seq(from=1,to=tf,by=timestep),Chlint,xout=(t+timestep/2))
  output<-ClamF_pop_equations(Param, Tapp$y, Chlapp$y, Ww[t]+k2/2)
  dw=unlist(output[1])
  k3=timestep*dw
  ...
  output<-ClamF_pop_equations(Param, Tapp, Chlapp, Ww[t]+k3)
  dw=unlist(output[1])
  k4=timestep*dw
  ...
  output<-ClamF_pop_equations(Param, Tint[t+timestep], Chlint[t+timestep], Ww[t+timestep])
